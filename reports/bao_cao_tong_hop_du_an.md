...
Baseline PyTorch Lightning gồm:
...
Mô hình có khoảng **268 nghìn tham số** và dùng `pack_padded_sequence` để bỏ qua padding. Tuy nhiên, pipeline này khác đáng kể:

- `MAX_LEN = 400`;
- lowercase và giải mã URL/HTML;
- ghép `raw [SEP] normalized`;
- gộp cả 150.000 payload obfuscation vào dữ liệu rồi chia ngẫu nhiên 70/15/15.

Do đó, kết quả LSTM-only chỉ được trình bày như baseline hệ thống độc lập, không dùng để kết luận riêng về ưu/nhược điểm kiến trúc LSTM.
