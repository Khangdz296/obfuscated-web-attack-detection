...
CNN-LSTM song song vẫn là baseline độc lập vì cách chia tập khác. LSTM-only hiện dùng cùng hai nguồn dữ liệu, chính sách no-decoding và tập obfuscation tách ngoài quá trình train; tuy nhiên, đây vẫn chưa phải ablation tuyệt đối vì framework, regularization và lịch huấn luyện khác các mô hình Keras ở mục 8.1.
...
CNN-LSTM song song khác cách chia tập và độ dài chuỗi nên không thể dùng cho ablation kiến trúc trực tiếp. LSTM-only đã được cập nhật về cùng hai nguồn dữ liệu, quy tắc no-decoding, khử trùng lặp, split 72/8/20 và tập obfuscation tách ngoài train. Điều này làm phép so sánh với nhóm mô hình chính có ý nghĩa hơn. Tuy nhiên, khác biệt PyTorch/Keras, callback, regularization và hyperparameter vẫn là biến nhiễu; muốn ablation tuyệt đối cần giữ cố định toàn bộ các yếu tố này ngoài kiến trúc.
