...
\[\text{label} =\begin{cases}1, & \hat{y} \ge t \\0, & \hat{y} < t\end{cases}\]
...
Đây là baseline độc lập, không phải đầu ra CNN đưa vào LSTM. Hai nhánh dùng chung embedding:

                        -> CNN blocks -> GlobalMaxPool --┐
    Embedding -------------------------------------------+-> Concatenate -> Dense -> Sigmoid
                        -> LSTM(32) -> LSTM(16) ---------┘

Mô hình có **165.569 tham số**. Pipeline riêng loại 30 payload có nhãn mâu thuẫn, chia 70/15/15 và dùng `MAX_LEN = 512`. Tập obfuscation tự tạo không được đánh giá trong notebook này.

Lưu ý: một số mô tả cũ ghi BiLSTM, nhưng code thực nghiệm sử dụng hai LSTM một chiều 32 và 16 units.

### 7.5. LSTM-only của thành viên khác
...
Pipeline sử dụng hai tập `SQLInjection_XSS_MixDataset.1.0.0.csv` và `csic_database.csv`. Payload được giữ nguyên chữ hoa/thường và encoding; bước chuẩn hóa chỉ gộp khoảng trắng liên tiếp và loại khoảng trắng ở đầu/cuối. Sau khi bỏ payload rỗng, xóa payload trùng và loại 30 payload có nhãn mâu thuẫn, dữ liệu còn **177.217 mẫu** (63.294 normal, 113.923 attack). Dữ liệu được stratified split thành **72% train, 8% validation và 20% test**; vocabulary ký tự chỉ được fit trên train (187 ký tự) và `MAX_LEN = 512`.
...
Ngưỡng 0,18 tăng Attack Recall và giảm 32 false negative so với ngưỡng 0,5, nhưng tăng false positive từ 49 lên 62. Đây là baseline hệ thống độc lập; vì nguồn dữ liệu, tiền xử lý và cách chia tập khác nhóm mô hình ở các mục trước, không dùng kết quả này để kết luận trực tiếp về ưu/nhược điểm của kiến trúc LSTM so với CNN-LSTM.
...
### 8.3. Baseline độc lập của thành viên khác

| Mô hình | Giao thức | Accuracy | Attack Precision | Attack Recall | Attack F1 | ROC-AUC | FP  | FN  |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| CNN-LSTM song song | Hai tập cơ sở, split 70/15/15, max length 512 | 98,1605% | 99,4342% | 97,6943% | 98,5565% | 99,3790% | 95  | 394 |
| LSTM-only, threshold 0,5 | Obfuscation đã trộn trước split, max length 400 | 99,2049% | 99,9032% | 99,1101% | 99,5050% | 99,7818% | 38  | 352 |
| LSTM-only, threshold 0,38 | Ngưỡng chọn trên validation | 99,2069% | 99,8904% | 99,1252% | 99,5064% | 99,7818% | 43  | 346 |

Không đặt các số liệu trong bảng này ngang hàng tuyệt đối với nhóm 8.1 vì dữ liệu test và preprocessing không giống nhau. Đặc biệt, LSTM-only đã thấy nhiều biến thể obfuscation trong train.
...
### 9.5. Hai baseline độc lập không thể dùng cho ablation kiến trúc

CNN-LSTM song song khác cách chia tập và độ dài chuỗi. LSTM-only còn thay đổi cả preprocessing và đưa obfuscation vào train. Nếu kết quả khác nhau, không thể quy toàn bộ nguyên nhân cho kiến trúc. Muốn đánh giá riêng tác động của LSTM, cần chạy LSTM-only bằng chính pipeline chung.
...
5. Chưa có LSTM-only dùng hoàn toàn cùng pipeline để ablation công bằng.
...
4. Xây dựng LSTM-only dùng đúng pipeline chung.
